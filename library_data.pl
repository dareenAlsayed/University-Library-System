...
topics(prolog_fundamentals, [tracing,facts, rules, queries, unification]).
topics(recursion_in_depth, [base_case, recursive_case, tracing, termination]).
topics(list_programming, [head_tail, member, append,rules,length, prefix, suffix]).
topics(ai_intro, [search, logic,tracing, knowledge_representation]).
